...
def get_game_times_from_steam(unplayed_games, max_games=10, delay_between_requests=2):
    game_data = []
    games_checked = 0
...
def index():
    ...
    if request.method == 'POST':
        ...
        if form_type == 'search_game':
        ...
        elif form_type == 'calculate_unplayed':
            # Calculate for selected unplayed games
            steam_id = request.form.get('steam_id')
            free_time = request.form.get('free_time')
            free_time_unit = request.form.get('free_time_unit')
            selected_games = request.form.getlist('selected_games')
            if not free_time or not free_time_unit or not steam_id:
                error = "Please fill out all fields."
            else:
                try:
                    free_time = float(free_time)
            ...
            if not error:
                if selected_games:
                    selected_games = selected_games[:10]
                    user_unplayed_data = get_game_times_from_steam(selected_games, max_games=10)
